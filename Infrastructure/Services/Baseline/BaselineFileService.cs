...
{
    // Constants
    private const float _vOLTAGE_FACTOR = (5.0f / 16383.0f) * 1000.0f;
    private const int _cHUNK_SIZE = 4128;
    ...
    public async Task<Result<List<BaselineData>>> ProcessFileStreamAsync(string filePath, IProgress<double>? progress = null)
    {
        ...
        try
        {
            ...
            int estimatedCapacity = (int)Math.Min(fileSize / (_cHUNK_SIZE * 2), 100000);
            ...
            {
                var fileBuffer = new char[131072];
                StringBuilder hexAccumulator = new(_cHUNK_SIZE * 4);
                ...
                var l1l2Dec = arrayPool.Rent(BUFFER_SIZE);
                var l6l7Dec = arrayPool.Rent(BUFFER_SIZE);
    ...
    private static void ProcessAccumulatedHex(StringBuilder sb, List<BaselineData> results, int[] l1l2Dec, int[] l6l7Dec, bool force = false)
    {
        ...
        while (searchIndex < bufferStr.Length)
        {
            ...
            if (headerIndex + _cHUNK_SIZE <= bufferStr.Length)
            {
                var segmentSpan = bufferStr.AsSpan(headerIndex, _cHUNK_SIZE);
                ProcessSingleSegment(segmentSpan, results, l1l2Dec, l6l7Dec);
                searchIndex = headerIndex + _cHUNK_SIZE;
            }
            else
            {
                sb.Remove(0, searchIndex);
    ...
    {
        var samplingPacket = ExtractSamplingPacket(segmentSpan);
        ...
        for (var i = 0; i < SAMPLES_PER_SEGMENT; i++)
        {
            ...
            var l1l2Offset = 36 + 64 * i * 2;
            var l6l7Offset = 1956 + 64 * i * 2;
    ...
    {
        var byte1 = HexCharToInt(hexDataSpan[32]) * 16 + HexCharToInt(hexDataSpan[33]);
        var byte2 = HexCharToInt(hexDataSpan[34]) * 16 + HexCharToInt(hexDataSpan[35]);
    ...
    {
        for (var j = 0; j < AppConstants.ChannelsPerLayer; j++)
        {
            var j2 = j * 2;
            var j2_32 = j2 + 32;

            // L1
            var l1Val = (l1l2Dec[j2] << 8) | l1l2Dec[j2 + 1];
            data.L1[j] = l1Val;
            data.L1_Voltage[j] = l1Val * _vOLTAGE_FACTOR;

            // L2
            var l2Val = (l1l2Dec[j2_32] << 8) | l1l2Dec[j2_32 + 1];
            data.L2[j] = l2Val;
            data.L2_Voltage[j] = l2Val * _vOLTAGE_FACTOR;

            // L6
            var l6Val = (l6l7Dec[j2] << 8) | l6l7Dec[j2 + 1];
            data.L6[j] = l6Val;
            data.L6_Voltage[j] = l6Val * _vOLTAGE_FACTOR;

            // L7
            var l7Val = (l6l7Dec[j2_32] << 8) | l6l7Dec[j2_32 + 1];
            data.L7[j] = l7Val;
            data.L7_Voltage[j] = l7Val * _vOLTAGE_FACTOR;
        }
    }
    ...
    public async Task<Result> SaveToExcelAsync(List<BaselineData> dataList, string filePath, IProgress<double>? progress = null)
    {
        ...
        try
        {
            ...
            if (rowCount > 0)
            {
                var colCount = 2 + (AppConstants.ChannelsPerLayer * 4);
                ...
                for (var i = 0; i < rowCount; i++)
                {
                    ...
                    var c = 2;
                    for (var j = 0; j < AppConstants.ChannelsPerLayer; j++) dataArray[i, c++] = item.L1[j];
                    for (var j = 0; j < AppConstants.ChannelsPerLayer; j++) dataArray[i, c++] = item.L2[j];
                    for (var j = 0; j < AppConstants.ChannelsPerLayer; j++) dataArray[i, c++] = item.L6[j];
                    for (var j = 0; j < AppConstants.ChannelsPerLayer; j++) dataArray[i, c++] = item.L7[j];
    ...
    private static void WriteHeaders(ExcelWorksheet ws)
    {
        ...
        for (var i = 1; i <= AppConstants.ChannelsPerLayer; i++)
        {
    ...
    public async Task<Result<List<BaselineData>>> ReadExcelFileAsync(string filePath, IProgress<double>? progress = null)
    {
        ...
        try
        {
            ...
            for (var r = 0; r < dataRows; r++)
            {
                ...
                var c = 2;
                for (var i = 0; i < AppConstants.ChannelsPerLayer; i++)
                {
                    var val = rawValues[r, c] != null ? Convert.ToInt32(rawValues[r, c]) : 0;
                    data.L1[i] = val;
                    data.L1_Voltage[i] = val * _vOLTAGE_FACTOR;
                    c++;
                }
                for (var i = 0; i < AppConstants.ChannelsPerLayer; i++)
                {
                    var val = rawValues[r, c] != null ? Convert.ToInt32(rawValues[r, c]) : 0;
                    data.L2[i] = val;
                    data.L2_Voltage[i] = val * _vOLTAGE_FACTOR;
                    c++;
                }
                for (var i = 0; i < AppConstants.ChannelsPerLayer; i++)
                {
                    var val = rawValues[r, c] != null ? Convert.ToInt32(rawValues[r, c]) : 0;
                    data.L6[i] = val;
                    data.L6_Voltage[i] = val * _vOLTAGE_FACTOR;
                    c++;
                }
                for (var i = 0; i < AppConstants.ChannelsPerLayer; i++)
                {
                    var val = rawValues[r, c] != null ? Convert.ToInt32(rawValues[r, c]) : 0;
                    data.L7[i] = val;
                    data.L7_Voltage[i] = val * _vOLTAGE_FACTOR;
                    c++;
                }
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            // No managed resources to release currently.
            // The _disposed flag prevents further use after disposal.
            _disposed = true;
        }
    }
