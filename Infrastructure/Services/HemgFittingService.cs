    ...
    public class HemgFittingService
    {
        ...
        public (double[] fitCurve, double[] parameters) HemgDoubleSidedFit(double[] binCenters, double[] counts)
        {
            try
            {
                ...
                // --- Improved Amplitude Guess ---
                // The HEMG model returns PDF-like values. We need to find the scaling factor
                // to match the histogram's max counts.
                double modelAtPeak = HyperEmgDouble(mu0, 1.0, mu0, sigma0, new[] { tauL0 }, new[] { 0.5 }, new[] { tauR0 }, new[] { 0.5 });
                double A0 = (modelAtPeak > 1e-15) ? maxCounts / modelAtPeak : maxCounts;
                ...
                double[] ub = new[] { A0 * 10.0, binCenters[binCenters.Length - 1], sigma0 * 5, sigma0 * 10, sigma0 * 10, 1.0, 1.0 };
