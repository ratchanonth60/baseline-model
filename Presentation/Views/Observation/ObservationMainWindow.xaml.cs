...
public partial class ObservationMainWindow : Window
{
    ...
    private const string _fORMAT_DATE = "yyyy-MMM-dd HH:mm:ss.fff";
    ...
    public ObservationMainWindow(ObservationViewModel viewModel, IObservationExcelHelper excelHelper)
    {
        ...
        _timer.Tick += (s, e) => DateTimeLabel.Text = DateTime.Now.ToString(_fORMAT_DATE);
        _timer.Start();
    ...
    private async void BtnReadData_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            TxtDataCount.Text = $"{_totalSteps.ToString(System.Globalization.CultureInfo.InvariantCulture)}";
            TxtParticleCount.Text = $"{(_totalSteps * 5).ToString(System.Globalization.CultureInfo.InvariantCulture)}";
            ...
            while (_data <= _totalSteps && !_stopFlag)
            {
                ...
                if (isFirstData)
                {
                    TxtStartTime.Text = _viewModel.DataProcessor.GetDateTimeFromHexData(hexData).ToString(_fORMAT_DATE, System.Globalization.CultureInfo.InvariantCulture);
                    isFirstData = false;
                }
            ...
            if (_totalSteps > 0)
            {
                ...
                TxtStopTime.Text = _viewModel.DataProcessor.GetDateTimeFromHexData(lastHexData).ToString(_fORMAT_DATE, System.Globalization.CultureInfo.InvariantCulture);
            }
    ...
    private void ProcessHeader(string[] hexData)
    {
        ...
        string checksum_hex = (total_sum % 65536).ToString("X4", System.Globalization.CultureInfo.InvariantCulture);
    ...
    private void CmbDSSDLayer_SelectionChanged(object sender, SelectionChangedEventArgs e) => RefreshDSSDPlots();

    private void TxtDSSDAxisChanged(object sender, TextChangedEventArgs e) => RefreshDSSDPlots();

    private void ChkDSSDFit_Changed(object sender, RoutedEventArgs e) => RefreshDSSDPlots();

    private void CmbDSSDFitMethod_SelectionChanged(object sender, SelectionChangedEventArgs e) => RefreshDSSDPlots();
    ...
    private void TxtBGOAxisChanged(object sender, TextChangedEventArgs e) => RefreshBGOPlots();
        ...
        TextBlock rmsLabel, TextBlock fwhmLabel, TextBlock resLabel)
    {
        ...
        // Reset stats
        if (peakLabel != null) peakLabel.Text = _nA;
        if (countsLabel != null) countsLabel.Text = _nA;
        if (meanLabel != null) meanLabel.Text = _nA;
        if (rmsLabel != null) rmsLabel.Text = _nA;
        if (fwhmLabel != null) fwhmLabel.Text = _nA;
        if (resLabel != null) resLabel.Text = _nA;
