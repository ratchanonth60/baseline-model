...
namespace BaselineMode.WPF.Presentation.ViewModels.Baseline
{
    public partial class MainViewModel : ObservableObject, IDisposable
    {
        ...
        /// </summary>
        private Func<BaselineData, float[]> GetLayerSelector() => SelectedLayerIndex switch
        {
            1 => d => d.L2,
            2 => d => d.L6,
            3 => d => d.L7,
        ...
        /// Converts float data to double for processing.
        /// </summary>
        private double[] ExtractChannelData(Func<BaselineData, float[]> layerSelector, int channelIndex)
        {
            int count = ProcessedData.Count;
            double[] data = new double[count];
            for (int i = 0; i < count; i++)
                data[i] = (double)layerSelector(ProcessedData[i])[channelIndex];
            return data;
        }
